...
public abstract class ByteToCharConverter {
	 ...
	 *                if the input buffer contains any sequence of bytes that is
	 *                illegal for the input character set.
	 * @exception UnknownCharacterException
	 *                for any character that that cannot be converted to
	 *                Unicode. Thrown only when converter is not in substitution
	 *                mode.
	 * @exception ConversionBufferFullException
	 *                if output array is filled prior to converting all the
	 *                input.
	 */
	 ...
	 * @param input
	 *            array containing Unicode characters to be converted.
	 * @exception MalformedInputException
	 *                if the input buffer contains any sequence of chars that is
	 *                illegal in the input character encoding. After this
	 *                exception is thrown, the method nextByteIndex can be
	 *                called to obtain the index of the first invalid input byte
	 *                and getBadInputLength can be called to determine the
	 *                length of the invalid input.
	 *
	 * @see #nextByteIndex
	...
	public char[] convertAll(byte input[]) throws MalformedInputException {
		...
		try {
			int outputLength = convert(input, 0, input.length, output, 0,
					output.length);
	 ...
	 *                multibyte character sequence. flush will write what it can
	 *                to the output buffer and reset the converter before
	 *                throwing this exception. An additional call to flush is
	 *                not required.
	 * @exception ConversionBufferFullException
	 *                if output array is filled before all the output can be
	 *                flushed. flush will write what it can to the output buffer
	 *                and remember its state. An additional call to flush with a
	 *                new output buffer will conclude the operation.
	 */
	 ...
	 *
	 * @param doSub
	 *            if true, enable substitution mode.
	 ...
	 *                if given byte array is longer than the value returned by
