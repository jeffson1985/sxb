package org.sxb.plugin.quartz.job;

import org.sxb.plugin.quartz.QuartzKey;
import org.sxb.plugin.quartz.Quartzer;
...
/**
 * Created by son on 14/11/29.
 */
public class QuartzCronJob<M extends Job> extends QuartzJob<M> {

	private String cron;

	public QuartzCronJob(String name, String cron, Class<M> jobClass) {
		this(Quartzer.nextKey(name), cron, jobClass);
	}

	public QuartzCronJob(String group, String name, String cron,
			Class<M> jobClass) {
		this(Quartzer.nextKey(group, name), cron, jobClass);
	}

	public QuartzCronJob(QuartzKey quartzKey, String cron, Class<M> jobClass) {
	...
	/**
	 * @param force
	 *            是否强制启动，true 表示 如果存在相同的key值的任务时，停止任务，强制执行新任务
	 */
