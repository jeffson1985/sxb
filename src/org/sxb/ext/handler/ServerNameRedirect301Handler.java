...
http://cetvision.com redirect 301 to  http://www.cetvision.com

<%@ page language="java" pageEncoding="utf-8"%>
<%@taglib prefix="s" uri="/struts-tags"%>
<%
if(request.getRequestURL().indexOf("http://cetvision.com") >= 0) {
	// String requestURI = request.getRequestURI();
	// String queryString = request.getQueryString();
	// queryString = (queryString == null ? "" : "?" + queryString);
	//attempt to merge non-www urls
	response.setStatus(301);	// 301 rewrite
	// response.setHeader("Location", "http://www.cetvision.com" + requestURI + queryString);
	response.setHeader("Location", "http://www.cetvision.com");
	response.setHeader("Connection", "close");
}
else {%>
