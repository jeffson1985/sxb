...
import org.sxb.convert.sp.SerializableTypeWrapper.FieldTypeProvider;
import org.sxb.convert.sp.SerializableTypeWrapper.MethodParameterTypeProvider;
import org.sxb.convert.sp.SerializableTypeWrapper.TypeProvider;
...
public final class ResolvableType implements Serializable {
	...
	private boolean isAssignableFrom(ResolvableType other, Map<Type, Type> matchedBefore) {
		...
		if (exactMatch ? !ourResolved.equals(otherResolved) : !ClassKits.isAssignable(ourResolved, otherResolved)) {
			return false;
		}
	...
	public ResolvableType as(Class<?> type) {
		...
		if (ObjectKits.nullSafeEquals(resolve(), type)) {
			return this;
		}
	...
	public ResolvableType[] getInterfaces() {
		Class<?> resolved = resolve();
		if (resolved == null || ObjectKits.isEmpty(resolved.getGenericInterfaces())) {
	...
	private Type resolveBounds(Type[] bounds) {
		if (ObjectKits.isEmpty(bounds) || Object.class.equals(bounds[0])) {
			return null;
		}
		return bounds[0];
	}
	...
	private ResolvableType resolveVariable(TypeVariable<?> variable) {
		...
		if (this.type instanceof ParameterizedType) {
			...
			for (int i = 0; i < variables.length; i++) {
				if (ObjectKits.nullSafeEquals(variables[i].getName(), variable.getName())) {
	...
	public boolean equals(Object other) {
		...
		return (ObjectKits.nullSafeEquals(this.type, otherType.type) &&
				ObjectKits.nullSafeEquals(getSource(), otherType.getSource()) &&
				variableResolverSourceEquals(otherType.variableResolver) &&
				ObjectKits.nullSafeEquals(this.componentType, otherType.componentType));
	}

	@Override
	public int hashCode() {
		int hashCode = ObjectKits.nullSafeHashCode(this.type);
		hashCode = 31 * hashCode + ObjectKits.nullSafeHashCode(getSource());
		hashCode = 31 * hashCode + variableResolverSourceHashCode();
		hashCode = 31 * hashCode + ObjectKits.nullSafeHashCode(this.componentType);
		return hashCode;
	}
	...
	private boolean variableResolverSourceEquals(VariableResolver other) {
		...
		return ObjectKits.nullSafeEquals(this.variableResolver.getSource(), other.getSource());
	}

	private int variableResolverSourceHashCode() {
		int hashCode = 0;
		if (this.variableResolver != null) {
			hashCode = ObjectKits.nullSafeHashCode(this.variableResolver.getSource());
		}
		return hashCode;
	}
	...
	public String toString() {
		...
		if (hasGenerics()) {
			result.append('<');
			result.append(StringKits.arrayToDelimitedString(getGenerics(), ", "));
			result.append('>');
		}
