...
import org.sxb.convert.sp.AnnotationUtils;
...
public class TypeDescriptor implements Serializable {
	...
	private static final boolean streamAvailable = ClassKits.isPresent(
	...
	public Class<?> getObjectType() {
		return ClassKits.resolvePrimitiveIfNecessary(getType());
	}
	...
	public String getName() {
		return ClassKits.getQualifiedName(getType());
	}
	...
	public boolean equals(Object obj) {
		...
		if (!ObjectKits.nullSafeEquals(this.type, other.type)) {
			return false;
		}
		...
		if (isCollection() || isArray()) {
			return ObjectKits.nullSafeEquals(getElementTypeDescriptor(), other.getElementTypeDescriptor());
		}
		else if (isMap()) {
			return ObjectKits.nullSafeEquals(getMapKeyTypeDescriptor(), other.getMapKeyTypeDescriptor()) &&
					ObjectKits.nullSafeEquals(getMapValueTypeDescriptor(), other.getMapValueTypeDescriptor());
		}
		else {
			return true;
		}
	...
	private static class StreamDelegate {
		...
		public static boolean isStream(Class<?> type) {
			boolean  rst = false;
			try {
				rst = ClassKits.forName("java.util.stream.Stream", TypeDescriptor.class.getClassLoader()).isAssignableFrom(type);
		...
		public static TypeDescriptor getStreamElementType(TypeDescriptor source) {
			Class<?>  clazz = null;
			try {
				clazz = ClassKits.forName("java.util.stream.Stream", TypeDescriptor.class.getClassLoader());
			} catch (ClassNotFoundException | LinkageError e) {
			
				e.printStackTrace();
			}
			return getRelatedIfResolvable(source, source.resolvableType.as(clazz).getGeneric());
