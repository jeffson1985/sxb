...
class JettyServer implements IServer {
	...
	private void doStart() {
		...
		System.out.println("Starting Sxb " + Const.SXB_VERSION);
		server = new Server();
			...
			Scanner scanner = new Scanner(PathKit.getRootClassPath(), scanIntervalSeconds) {
				public void onChange() {
					try {
						System.err.println("\nLoading changes ......");
						webApp.stop();
						SxbClassLoader loader = new SxbClassLoader(webApp, getClassPath());
		...
		try {
			...
			server.start();
			System.out.println("Starting Complete. Welcome To The Sxb World :)");
			server.join();
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(100);
	...
	private void changeClassLoader(WebAppContext webApp) {
		try {
			...
			SxbClassLoader wacl = new SxbClassLoader(webApp, classPath);
