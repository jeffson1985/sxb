 ...
 */
package org.sxb.data.redis.listener;
...
import org.sxb.core.task.SimpleAsyncTaskExecutor;
import org.sxb.core.task.TaskExecutor;
import org.sxb.data.redis.RedisConnectionFailureException;
import org.sxb.data.redis.connection.ConnectionUtils;
import org.sxb.data.redis.connection.Message;
...
import org.sxb.data.redis.connection.util.ByteArrayWrapper;
...
import org.sxb.kit.ErrorHandler;
...
public class RedisMessageListenerContainer implements InitializingBean, DisposableBean, BeanNameAware, SmartLifecycle {
	 ...
	 */
	public static final String DEFAULT_THREAD_NAME_PREFIX = ClassKits.getShortName(RedisMessageListenerContainer.class)
			+ "-";
	 ...
	 * <p>
	 * The default implementation builds a {@link org.sxb.task.SimpleAsyncTaskExecutor} with the
	 * specified bean name (or the class name, if no bean name specified) as thread name prefix.
	 * 
	 * @see org.sxb.task.SimpleAsyncTaskExecutor#SimpleAsyncTaskExecutor(String)
	 */
	...
	private void initMapping(Map<? extends MessageListener, Collection<? extends Topic>> listeners) {
		...
		if (!CollectionKits.isEmpty(listeners)) {
	...
	private void removeListener(MessageListener listener, Collection<? extends Topic> topics) {
		...
		if (listener == null && CollectionKits.isEmpty(topics)) {
			subscriptionTask.cancel();
			return;
		}
		...
		if (CollectionKits.isEmpty(topics)) {
	...
	private class SubscriptionTask implements SchedulingAwareRunnable {
		 ...
		 * 
		 * @author Thomas Darimont
		 * @see org.sxb.data.redis.listener.RedisMessageListenerContainer.SubscriptionTask.SubscriptionPresentTestCondition
		 */
		private class PatternSubscriptionPresentCondition extends SubscriptionPresentCondition {

			@Override
			public boolean passes() {
				return super.passes() && !CollectionKits.isEmpty(connection.getSubscription().getPatterns());
			}
		}

		private byte[][] unwrap(Collection<ByteArrayWrapper> holders) {
			if (CollectionKits.isEmpty(holders)) {
				return new byte[0][];
			}
	 ...
	 * {@link org.sxb.data.redis.listener.RedisMessageListenerContainer.SpinBarrier}
	 * 
	 * @author Jennifer Hickey
	 * @author Thomas Darimont Note: Placed here to avoid API exposure.
	 */
