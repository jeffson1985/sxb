...
public class FreeMarkerRender extends Render {
	...
	public static boolean isReadTempFromJar = false;
    ...
    static void init(ServletContext servletContext, Locale locale, int template_update_delay) {
        ...
        // Make freemaker can read template from jar files
        // if can not found template in /WEB-INF/templates , it will search if from jar files
        if(isReadTempFromJar){
        	TemplateLoader t = config.getTemplateLoader();
        	ClassTemplateLoader ct = new ClassTemplateLoader(FreeMarkerRender.class, "/");
        	MultiTemplateLoader multi = new MultiTemplateLoader(new TemplateLoader[]{t,ct});
