...
import static org.sxb.core.Const.DEFAULT_FILE_RENDER_BASE_PATH;
...
public class RenderFactory {
	...
	/**
	 * freemarker renderを初期化する<br>
	 * 初始化freemarker Render<br>
	 * @param servletContext
	 */
	...
	/**
	 * velocity renderを初期化する<br>
	 * 初始化velocity Render<br>
	 * @param servletContext
	 */
	...
	private void initJspRender(ServletContext servletContext) {
		try {
			...
			org.sxb.plugin.activerecord.ModelRecordElResolver.init(servletContext);
		}
		catch (ClassNotFoundException e) {
	...
	/**
	 * Return Render by default ViewType which config in SxbConfig
	 */
	...
	public Render getCsvRender(List<String> headers, List<?> data){
		return new CsvRender(headers, data);
	}
	
	public Render getReportRender(String view, ReportType format){
		//return new JasperRender(view);
		return new ReportRender(view, format);
	}
