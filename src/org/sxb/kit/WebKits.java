...
import org.sxb.web.util.UriComponents;
import org.sxb.web.util.UriComponentsBuilder;
...
public abstract class WebKits {
	...
	public static final String SESSION_MUTEX_ATTRIBUTE = WebKits.class.getName() + ".MUTEX";

	private static final Logger logger = Logger.getLogger(WebKits.class);
	...
	public static void setWebAppRootSystemProperty(ServletContext servletContext) throws IllegalStateException {
		...
		if (oldValue != null && !StringKits.pathEquals(oldValue, root)) {
			throw new IllegalStateException(
	...
	public static Boolean getDefaultHtmlEscape(ServletContext servletContext) {
		...
		return (StringKits.hasText(param) ? Boolean.valueOf(param) : null);
	}
	...
	public static int getTargetPage(ServletRequest request, String paramPrefix, int currentPage) {
		...
		while (paramNames.hasMoreElements()) {
			...
			if (paramName.startsWith(paramPrefix)) {
				for (int i = 0; i < WebKits.SUBMIT_IMAGE_SUFFIXES.length; i++) {
					String suffix = WebKits.SUBMIT_IMAGE_SUFFIXES[i];
					if (paramName.endsWith(suffix)) {
	...
	public static MultiValueMap<String, String> parseMatrixVariables(String matrixVariables) {
		...
		if (!StringKits.hasText(matrixVariables)) {
			return result;
		}
		...
		while (pairs.hasMoreTokens()) {
			...
			if (index != -1) {
				...
				for (String value : StringKits.commaDelimitedListToStringArray(rawValue)) {
	...
	/**
	 * Detect the port of the given uri component
	 * @param component
	 * @return
	 */
