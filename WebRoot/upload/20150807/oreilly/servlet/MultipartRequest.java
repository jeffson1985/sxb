...
public class MultipartRequest {
	...
	protected Hashtable<Object, Vector<String>> parameters = new Hashtable<Object, Vector<String>>(); // name
																										// -
																										// Vector
																										// of
																										// values
	...
	public MultipartRequest(HttpServletRequest request, String saveDirectory,
		...
		while ((part = parser.readNextPart()) != null) {
			...
			if (part.isParam()) {
				...
				//System.out.println("Sxb Test :" + name);
				if (fileName != null) {
					filePart.setRenamePolicy(policy); // null policy is OK
					// The part actually contained a file
					filePart.writeTo(dir);
					name = getKeyByFileCount(name);
					files.put(
							name,
	...
	// 根据上传文件数来修正保存上传文件信息的Hashtable的键值
	// 为了对应Html5中 multi 属性可以同时选择多文件上传
	private String getKeyByFileCount(final String key){
		String keyName = key;
		if (files.containsKey(key)) {
			 keyName += files.size() + 1;
		}
		return keyName;
	}
